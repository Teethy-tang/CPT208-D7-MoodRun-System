import type { AvatarConfig } from '../../types/moodrun';
...
} satisfies Record<keyof AvatarConfig, string[]>;
...
export function getAvatarLabel(key: keyof AvatarConfig, value: string) {
  return optionLabels[key]?.[value] || String(value).toUpperCase();
}

export function normalizeAvatar(avatar: Partial<AvatarConfig> = {}): AvatarConfig {
  const normalized = {} as AvatarConfig;

  (Object.keys(defaultAvatar) as Array<keyof AvatarConfig>).forEach((key) => {
    const value = avatar[key];
    normalized[key] = avatarOptions[key].includes(String(value)) ? String(value) : defaultAvatar[key];
  });
...
export function loadAvatar() {
  try {
    return normalizeAvatar(JSON.parse(localStorage.getItem(AVATAR_KEY) || '{}') as Partial<AvatarConfig>);
  } catch (error) {
...
export function saveAvatar(avatar: Partial<AvatarConfig>) {
  localStorage.setItem(AVATAR_KEY, JSON.stringify(normalizeAvatar(avatar)));
}

export function randomAvatar(): AvatarConfig {
  return Object.fromEntries(
    (Object.entries(avatarOptions) as Array<[keyof AvatarConfig, string[]]>).map(([key, values]) => {
...
function rects(items: number[][] = [], fill: string, opacity = 1) {
  return items
...
function getBodyRects({ body }: AvatarConfig) {
  if (body === 'block') {
    return {
      main: [
        [13, 15, 22, 22],
...
function getEyeRects({ eyes }: AvatarConfig) {
  if (eyes === 'sleepy') {
    return [
      [16, 23, 6, 2],
      [27, 23, 6, 2],
...
function getAntennaRects({ antenna }: AvatarConfig) {
  if (antenna === 'horns') {
    return {
      back: [
        [12, 7, 4, 7],
...
function getArmRects({ arms }: AvatarConfig) {
  if (arms === 'wave') {
    return {
      shadow: [
        [5, 19, 6, 13],
...
function getFeetRects({ feet }: AvatarConfig) {
  if (feet === 'boots') {
    return {
      shadow: [
        [15, 38, 7, 5],
...
function getAccessoryRects({ accessory }: AvatarConfig) {
  if (accessory === 'visor') {
    return [
      [16, 20, 17, 4],
      [20, 24, 9, 2],
