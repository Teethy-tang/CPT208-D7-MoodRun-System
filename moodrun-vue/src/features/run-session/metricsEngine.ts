import type { PositionLike, RoutePoint, RoutePreview, RunSnapshot, TrackingResult } from '../../types/moodrun';
...
export function createRunMetrics({ targetDistanceKm }: { targetDistanceKm: number }) {
  const startedAt = Date.now();
  const acceptedPoints: AcceptedPoint[] = [];
  const recentSegments: Segment[] = [];
  let currentPosition: NormalizedPoint | null = null;
  ...
  function addPosition(position: PositionLike, now = Date.now()): TrackingResult {
    ...
    const jitterThreshold = Math.max(
      MIN_SEGMENT_DISTANCE_METERS,
      Math.min(point.accuracy, MIN_ACCEPTED_ACCURACY_METERS) * 0.45,
    );
  ...
  function buildSnapshot(now = Date.now()): RunSnapshot {
  ...
  function buildRoutePreview(): RoutePreview {
    ...
    const projected = sourcePoints.map((point) => ({ ...projectToLocalMeters(point, reference) }));

    const displayPoint =
      currentPosition && currentPosition.accuracy <= MAX_ACCEPTABLE_ACCURACY_METERS
        ? currentPosition
...
function normalizePosition(position: PositionLike | null | undefined): NormalizedPoint | null {
  ...
  return {
    ...
    accuracy: Number.isFinite(coords?.accuracy) ? (coords?.accuracy as number) : MAX_ACCEPTABLE_ACCURACY_METERS + 1,
    timestamp: Number.isFinite(timestamp) ? (timestamp as number) : Date.now(),
  };
}

function haversineDistanceMeters(pointA: NormalizedPoint, pointB: NormalizedPoint) {
...
function projectToLocalMeters(point: NormalizedPoint, reference: NormalizedPoint): ProjectedPoint {
...
function getProjectedBounds(points: ProjectedPoint[]): Bounds {
  const initial = {
...
function scaleRoutePoint(point: ProjectedPoint, bounds: Bounds): RoutePoint {
...
function getGpsQuality(accuracy: number | null) {
  ...
  if ((accuracy as number) <= 12) return 'STRONG';
  if ((accuracy as number) <= 25) return 'GOOD';
  if ((accuracy as number) <= 40) return 'OK';
  return 'WEAK';
}
